...
Robotic manipulators have evolved over the years to fit the automation demand of the industry in a wide range of tasks that required the ability to sense, grasp and move objects of different size, weight and shape within a given work space. The next sections give a brief overview of the main components of robotic manipulators.
...
Cartesian robots have three linear axis of motion (diagram shown in \cref{fig:cartesian-robot}) and are used in operations that require mainly translations of the end effector (example in \cref{fig:cartesian-robot-festo-3d-gantry}), such as moving objects within large workplaces or spraying / milling / drilling / cutting material using \glspl{cnc} machines.

\begin{figure}[H]
	\begin{floatrow}[2]
		\ffigbox[\FBwidth]
		{\includegraphics[height=.145\textheight]{robots/cartesian-robot}}
		{\caption[Diagram of a Cartesian robot]{Diagram of a Cartesian robot\protect\footnotemark}\label{fig:cartesian-robot}}
		\ffigbox[\FBwidth]
		{\includegraphics[height=.145\textheight]{robots/cartesian-robot-festo-3d-gantry}}
	...
	\end{floatrow}
\end{figure}
\footnotetext[\the\numexpr\value{footnote}-1\relax]{\url{http://www.looptechnology.com/robotic-robot-types.asp}}
...
Cylindrical robots have two linear axis and one rotation axis around the origin (diagram shown in \cref{fig:cylindrical-robot}) and are useful for tasks such as sorting / packaging that require the movement of high payload packages (example in \cref{fig:cylindrical-robot-plate-crane}.

\begin{figure}[H]
	\begin{floatrow}[2]
		\ffigbox[\FBwidth]
		{\includegraphics[height=.155\textheight]{robots/cylindrical-robot}}
		{\caption[Diagram of a cylindrical robot]{Diagram of a cylindrical robot\protect\footnotemark}\label{fig:cylindrical-robot}}
		\ffigbox[\FBwidth]
		{\includegraphics[height=.155\textheight]{robots/cylindrical-robot-plate-crane}}
...
Polar / spherical robots have one linear axis and two perpendicular rotation axis (diagram shown in \cref{fig:polar-robot}) and can be used in welding / fettling operations (example in \cref{fig:polar-robot-fanuc}).

\begin{figure}[H]
	\begin{floatrow}[2]
		\ffigbox[\FBwidth]
		{\includegraphics[height=.138\textheight]{robots/polar-robot}}
		{\caption[Diagram of a polar robot]{Diagram of a polar robot\protect\footnotemark}\label{fig:polar-robot}}
		\ffigbox[\FBwidth]
...
\gls{scara} robots have one linear axis and two parallel rotation axis (diagram shown in \cref{fig:scara-robot}) and can perform quick pick and place tasks (example in \cref{fig:scara-robot-packaging}).

\begin{figure}[H]
	\begin{floatrow}[2]
		\ffigbox[\FBwidth]
		{\includegraphics[height=.125\textheight]{robots/scara-robot}}
		{\caption[Diagram of a \glsentrytext{scara} robot]{Diagram of a \glsentrytext{scara} robot\protect\footnotemark}\label{fig:scara-robot}}
		\ffigbox[\FBwidth]
		{\includegraphics[height=.125\textheight]{robots/scara-robot-packaging}}
		{\caption[\glsentrytext{scara} robot]{\glsentrytext{scara} robot\protect\footnotemark}\label{fig:scara-robot-packaging}}
	\end{floatrow}
\end{figure}
\footnotetext[\the\numexpr\value{footnote}-1\relax]{\url{http://www.looptechnology.com/robotic-robot-types.asp}}
...
Picker / delta / parallel robots have three parallelogram arms connected to the end effector in order to provide three translation axis and one rotation axis (diagram shown in \cref{fig:picker-parallel-robot}) and they can perform very fast pick and place tasks (example in \cref{fig:picker-parallel-robot-fanuc}).

\begin{figure}[H]
	\begin{floatrow}[2]
		\ffigbox[\FBwidth]
		{\includegraphics[height=.153\textheight]{robots/picker-parallel-robot}}
		{\caption[Diagram of a delta robot]{Diagram of a delta robot\protect\footnotemark}\label{fig:picker-parallel-robot}}
		\ffigbox[\FBwidth]
		{\includegraphics[height=.153\textheight]{robots/picker-parallel-robot-fanuc}}
		{\caption[Delta robot]{delta robot\protect\footnotemark}\label{fig:picker-parallel-robot-fanuc}}
	\end{floatrow}
\end{figure}
\footnotetext[\the\numexpr\value{footnote}-1\relax]{\url{http://www.looptechnology.com/robotic-robot-types.asp}}
...
Articulated robots are versatile arms with at least 3 rotation axis (diagram shown in \cref{fig:articulated-robot}) and are capable of performing a wide range of tasks, from welding to advanced assembly tasks (example in \cref{fig:articulated-robot-kuka}).

\begin{figure}[H]
	\begin{floatrow}[2]
		\ffigbox[\FBwidth]
		{\includegraphics[height=.2\textheight]{robots/articulated-robot}}
		{\caption[Diagram of an articulated robot]{Diagram of an articulated robot\protect\footnotemark}\label{fig:articulated-robot}}
		\ffigbox[\FBwidth]
		{\includegraphics[height=.2\textheight]{robots/articulated-robot-kuka}}
...
\begin{figure}[H]
	\begin{floatrow}[2]
		...
		{\caption{\glsentrytext{ccd} sensor\protect\footnotemark}\label{fig:ccd}}
		\ffigbox[\FBwidth]
		{\includegraphics[height=.21\textheight]{sensors/cmos}}
		{\caption{\glsentrytext{cmos} sensor\protect\footnotemark}\label{fig:cmos}}
	\end{floatrow}
\end{figure}
